...
def fn_dphidt(i,delay_number,phi_val,phi,Ap_val):
    ...
    if i-delay_number < 0:
        dphidt = (1/optimised_value_tau)*( -       1            + Ap_val*Q_norm*(1-phi_min) + phi_min )
    else:
        dphidt = (1/optimised_value_tau)*( -interpolate(phi,i-delay_number) + Ap_val*Q_norm*(1-phi_min) + phi_min )
...
def fn_dSoutdt(vessels):
    ...
    Q_tot = interpolate(pressure_difference,i)/R_tot
    Q_norm = Q_tot / baselines['Q in single(µm3/s)'][0]
    vessels['Q in single(µm3/s)'] = first_iteration['Q in single(µm3/s)']*network.at[i,'Q_norm']
    euler Sout
    euler Sin
     sort of steal from before dSindt
...
def RK4(i,phi_val,phi,delay_number):
    k1 = fn_dphidt(i,delay_number,phi_val,phi)
    k2 = fn_dphidt(i+0.5,,delay_number,phi_val + 0.5*k1*dt,phi
    k3 = fn_dphidt(i+0.5,,delay_number,phi_val + 0.5*k2*dt,phi)
    k4 = fn_dphidt(i+1,,delay_number,phi_val + k3*dt,phi)
...
def master(alpha,delay,baselines):
    ...
    for i in tqdm(range(len(t))):
        ...
        network.at[i,'Q_norm'] = network.at[i,'Q_tot'] / baselines_total_flow
...
#             vessels['Sav'] = vessels['Sav'] + vessels['dSavdt']*dt
        ...
        vessels['Q in single(µm3/s)'] = first_iteration['Q in single(µm3/s)']*network.at[i,'Q_norm']    # have just finished this.
