...
def single_RK_run(i,network_row,vessels,network):
    ...
    ks[5] = 0 # (1/tau_c)*(R- k*network_row['c'] - network_row['Q_norm']*network_row['c'] )
...
def update_v_n(the_change, multiplier, vessels, network_row):
    ...
    # print(the_change[0])
    network_row_new['Ap'] = network_row['Ap'] + multiplier* the_change[0]
    network_row_new['Dp'] = network_row['Dp'] + multiplier* the_change[1]
    network_row_new['An'] = network_row['An'] + multiplier* the_change[2]
    network_row_new['Dn'] = network_row['Dn'] + multiplier* the_change[3]
    network_row_new['phi'] = network_row['phi'] +multiplier* the_change[4]
    network_row_new['c'] = network_row['c'] +multiplier* the_change[5]

    vessels['tissue partials(mmHg)'] = vessels['tissue partials(mmHg)'] + multiplier* the_change[6]['dptdt']
    vessels['Saturation out'] = vessels['Saturation out'] + multiplier* the_change[7]['dSoutdt']
    vessels['Saturation in'] = vessels['Saturation in'] + multiplier* the_change[8]['dSindt']
...
def RK4(i,network,vessels):
    ...
    network_row_in = network.iloc[i,:].copy() # to take the shape of it and columns etc.
    vessels_temp_in = vessels.copy()  # could probably just load in first_iteration or a zeros version (may do this) as we recalculate everything. Not strictly true as we use some of the constants values. Could also use first_iteration as that helps with the flow stuff

    # no need to update
    k1s , network_row_TIMESTEP = single_RK_run( i , network_row_in , vessels_temp_in , network )

    if i == 1 or i ==2 or i ==3:
        print(network_row_TIMESTEP)


    network.iloc[i,:] = network_row_TIMESTEP

    vessels_temp,network_row = update_v_n(k1s,0.5,vessels_temp_in,network_row_TIMESTEP)
    k2s,ignore= single_RK_run(i+0.5,network_row,vessels_temp,network)

    vessels_temp,network_row = update_v_n(k2s,0.5,vessels_temp_in,network_row_TIMESTEP)
    k3s,ignore = single_RK_run(i+0.5,network_row,vessels_temp,network)

    vessels_temp,network_row = update_v_n(k3s,1,vessels_temp_in,network_row_TIMESTEP)
    k4s,ignore = single_RK_run(i+1,network_row,vessels_temp,network)

    change = combine_ks(k1s,k2s,k3s,k4s)

    return change,network_row_TIMESTEP
...
def master(alpha,delay,baselines):
    ...
    for i in tqdm(range(len(t)-1)): #Could maybe itterrows this column now as simply saving?
        ...
        delta_values , updated_network_step = RK4(i, network, vessels)

        vessels, network.iloc[i+1,:] = update_v_n(delta_values, 1, vessels, updated_network_step)
    ...
    return network,RK4_change_alltime
...
N,RK4_change_alltime = master(alpha,delay,baselines)
print('Run complete.')
