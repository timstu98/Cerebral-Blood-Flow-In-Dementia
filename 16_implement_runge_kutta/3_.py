...
def total_R(baselines,phi,alpha):
    ...
    baselines.at[6,'Resistance for Q'] = C_
...
def RK4(i,network,vessels):
    ...
    k2s= single_RK_run(i+0.5,network_row,vessels_temp,network)

    vessels_temp,network_row = update_v_n(k2s,0.5,vessels_temp_in,network_row_in)
    k3s = single_RK_run(i+0.5,network_row,vessels_temp,network)

    vessels_temp,network_row = update_v_n(k3s,1,vessels_temp_in,network_row_in)
    k4s = single_RK_run(i+1,network_row,vessels_temp,network)
...
def master(alpha,delay,baselines):
    ...
    for i in tqdm(range(len(t)-1)): #Could maybe itterrows this column now as simply saving?
        ...
        vessels, network.iloc[i+1,:] = update_v_n(change, 1, vessels, network.iloc[i,:] )
...
# network['dAndt'] = pd.Series(np.zeros(no)) # not necassarily
network['An'] = pd.Series(np.zeros(no))
# network['dDndt'] = pd.Series(np.zeros(no)) # not necassarily
