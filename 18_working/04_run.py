...
def network_initial_conditions(network):
    ...
    network_row0['kp_p'] = (m.kp_constant/2)*(1-np.tanh( (network_row0['pt_averaged']-pt_averaged_50) / pt_averaged_5))
    network_row0['kp_n'] = (m.kp_constant/2)*(1+np.tanh( (network_row0['pt_averaged']-pt_averaged_50) / pt_averaged_5 ))
...
def single_RK_run(i,network_row,vessels,network):
    ...
    network_row['kp_p'] = (m.kp_constant/2)*(1-np.tanh( (network_row['pt_averaged']-pt_averaged_50) / pt_averaged_5))
    network_row['kp_n'] = (m.kp_constant/2)*(1+np.tanh( (network_row['pt_averaged']-pt_averaged_50) / pt_averaged_5 ))
    ...
    for j in range(len(vessels)):
        ...
        # ks[7].at[j,'dSoutdt'] = 0.00001

        # if i == 4:
        #     print('j:',j)
        #     fun = 2/(np.pi*((vessels.at[j,'Diameter(µm)']**2)/4)*vessels.at[j,'Length(µm)']) \
        ...
        ks[7].at[j,'dSoutdt'] = fun

        # print('j:',j)
        # print(fun)
        # print(ks[7].at[j,'dSoutdt'])
        ...
        # print(Sat_out.at[j,'dSoutdt'])

        # vessels.at[j,'0'] = 2/(np.pi*((vessels.at[j,'Diameter(µm)']**2)/4)*vessels.at[j,'Length(µm)'])
        # vessels.at[j,'1'] = - (2 * np.pi * p.K * (vessels.at[j,'Diameter(µm)']/2) * constant.at[j,'Length(µm)'])/(vessels.at[j,'wall thickness(µm)']*p.cHb*p.H)
        # vessels.at[j,'2'] = (vessels.at[j,'partial pressure blood(mmHg)']-vessels.at[j,'tissue partials(mmHg)'])
        # vessels.at[j,'3'] = - vessels.at[j,'Q in single(µm3/s)']*(vessels.at[j,'Saturation out']-vessels.at[j,'Saturation in'])
        # vessels.at[j,'4'] = - ks[8].at[j,'dSindt']
...
def plt_n_t(column_string):
    ...
    plt.savefig(main_path / save_name)
...
# all_network, all_vessels, all_changes, all_k = {},{},{},{}
