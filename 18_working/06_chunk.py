...
def save_as_chunk(df,name,no_chunks):
    no = len(df) - 1
...
def small_n_com(network_row,changes):
    network_row['dphidt'] = changes[4]
    network_row['dApdt'] = changes[0]
    network_row['dDpdt'] = changes[1]
    network_row['dAndt'] = changes[2]
    network_row['dDndt'] = changes[3]
    network_row['dcdt'] = changes[5]
    return network_row

def small_v_com(vessels,changes):
    vessels['dptdt'] = changes[6]['dptdt']
    vessels['dSoutdt'] = changes[7]['dSoutdt']
    vessels['dSindt'] = changes['dSindt']
    return vessels
...
def combine_network_changes(network,RK4):
    ...
    network.to_csv(main_path / 'combined_network.csv',index=False)
...
def combine_vessels_changes(vessels_alltime,RK4):
    ...
    save_v.to_csv(main_path / 'combined_vessels.csv',index=False)
...
def master(m):
    ...
    for i in tqdm(range(m.no-1)):
        ...
        changes , network_row_all_updated, vessels_all_updated, ks = RK4(i, network_row_main_updated, vessels_main_updated,network)
        ks = 0

        for i == 0:
            small_n_com(network_row_all_updated,changes).to_csv(main_path / 'combined_network.csv',index=False)
            small_n_com(network_row_all_updated,changes).to_csv(main_path / 'combined_network.csv',index=False)
        else:
...
def save_stuff(network, all_vessels,all_changes,all_ks):
    ...
    txt_file = [
                ...
                'kp_constant: ' + str(m.kp_constant) + '\n',
                'kn_constant: ' + str(m.kn_constant) + '\n',
...
first_iteration_vessels = baseline.copy().loc[:,['Name','Number','Diameter(µm)','Length(µm)','Saturation in','Saturation out','Sav','Resistance for Q','Q in single(µm3/s)','partial pressure blood(mmHg)','X-Area wall(???)','wall thickness(µm)','tissue partials(mmHg)']]
...
save_stuff(network, all_vessels, all_changes, all_ks)
