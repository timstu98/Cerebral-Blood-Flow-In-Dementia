...
def update_v_n(the_change, multiplier, vessels, network_row,network):
    ...
    # vessels.to_csv('vessels.csv',index=False)

    # the_change[6].to_csv('the_change.csv',index=False)
    print(i)
    print(the_change[6])
...
def save_stuff(network, all_vessels,all_changes):
    ...
    txt_file = [
                ...
                'tau_c: 1/' + str(round(1/m.tau_c)) + '\n\n',

                'kp_constant: 1/' + str(round(1/m.kp_constant)) + '\n',
                ...
                'kn_constant: 1/' + str(round(1/m.kn_constant)) + '\n',
