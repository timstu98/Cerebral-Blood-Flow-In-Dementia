...
def master(alpha,delay,baselines):
    ...
    for i in tqdm(range(len(t))):
        ...
        if i-delay_number <0: # As cant index before 0 as not initialised for that.
            network.at[i,'dphidt'] =  (1/optimised_value_tau)*( - 1 + network.at[i,'Ap']*network.at[i,'Q_norm']*(1-network.at[i,'phi_min']) + network.at[i,'phi_min'] ) ###### Just added the An on this one as was not there before!
        else:
