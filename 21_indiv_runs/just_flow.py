...
class m:
    ...
    # kp_constant = 0.5*1/300 # s-1
    # kn_constant = 0.5*1/600 # s-1
...
def total_R(baselines,phi,alpha):
    C_ = baselines.at[6,'Resistance for Q']/2
...
def single_RK_run(i,network_row,vessels,network):
    ...
    network_row['phi_min'] = 0 #1-(m.alpha**0.25)*(1-p.phi_min_baseline)*(1-network_row['Q_norm'])**p.n
    ...
    network_row['kp_p'] = 0#(m.kp_constant/2)*(1-np.tanh( (network_row['pt_volume_averaged']-pt_averaged_50) / pt_averaged_5))
    network_row['kp_n'] = 0#(m.kp_constant/2)*(1+np.tanh( (network_row['pt_volume_averaged']-pt_averaged_50) / pt_averaged_5 ))
    ks[0]= 0# -network_row['kp_p']*network_row['Ap'] + network_row['kp_n']*(1-network_row['Ap']-network_row['Dp']) #dApdt
    ks[1] = 0# network_row['kp_p']*(1-network_row['Ap']-network_row['Dp']) #dDpdt
    
    network_row['kn_p'] = 0#(m.kn_constant/2)*(1+np.tanh( (network_row['c']-c_50) / c_5))
    network_row['kn_n'] = 0#(m.kn_constant/2)*(1-np.tanh( (network_row['c']-c_50) / c_5 ))
    ks[2] = 0 #-network_row['kn_p']*network_row['An'] + network_row['kn_n']*(1-network_row['An']-network_row['Dn']) #dAndt
    ks[3] = 0 #network_row['kn_p']*(1-network_row['An']-network_row['Dn']) #dDndt
    ...
    if i-delay_number <0: # As cant index before 0 as not initialised for that.
        ks[4] =  0 #(1/p.optimised_value_tau)*( -                    1                       + network_row['Ap']*(network_row['Q_norm']*(1-network_row['phi_min']) + network_row['phi_min']) )
    else:
        ks[4] =  0 #(1/p.optimised_value_tau)*( - interpolate(network['phi'],i-delay_number) + network_row['Ap']*(network_row['Q_norm']*(1-network_row['phi_min']) + network_row['phi_min']) )
