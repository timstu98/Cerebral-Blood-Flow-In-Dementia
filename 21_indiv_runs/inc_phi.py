...
def single_RK_run(i,network_row,vessels,network):
    ...
    if i-delay_number <0: # As cant index before 0 as not initialised for that.
        ks[4] =  (1/p.optimised_value_tau)*( -                    1                       + (network_row['Q_norm']*(1-network_row['phi_min']) + network_row['phi_min']) )
    else:
        ks[4] =  (1/p.optimised_value_tau)*( - interpolate(network['phi'],i-delay_number) + (network_row['Q_norm']*(1-network_row['phi_min']) + network_row['phi_min']) )
